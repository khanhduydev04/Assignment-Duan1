...
class Photo
{
  ...
  public function getAllPhotoByUser($id)
  {
    ...
    LEFT JOIN photos ON posts.id = photos.post_id WHERE users.id = '$id'";
  ...
  {
    $db = new connect();
    $sql = "SELECT photos.image_url FROM users INNER JOIN posts ON user.id = posts.user_id
    LEFT JOIN photos ON posts.id = photos.post_id WHERE user.id = '$id' AND photos.status = 'avatar'";
    return $db->pdo_query($sql);
  }
  ...
  public function getAllCoverByUser($id)
  {
    $db = new connect();
    $sql = "SELECT photos.image_url FROM users INNER JOIN posts ON user.id = posts.user_id
    LEFT JOIN photos ON posts.id = photos.post_id WHERE user.id = '$id' AND photos.status = 'cover'";
    return $db->pdo_query($sql);
  }
